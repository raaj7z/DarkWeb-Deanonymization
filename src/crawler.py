...
# CHANGED: thread locks, double-fetch fix, new module wiring, honest renames
import time
import random
import re
import json
...
from banner import grab_banner                  # ADDED
from tls import fetch_tls                       # ADDED
from server_status import ServerStatusChecker   # ADDED
from config import OUTPUT_DIR                   # ADDED
...
class DarkCrawler:
    ...
    def __init__(self, db=None, session_id=None,
        ...
        # ADDED: thread locks for concurrency safety
        ...
        info(f"DarkCrawler v3.1 | Workers: {max_workers} | Session: {session_id}")
    
    def _random_delay(self):
        time.sleep(random.uniform(*self.delay))
    
    def _refresh_session(self):
        # ADDED: lock so concurrent threads don't clobber the session mid-request
    ...
    def _maybe_rotate_circuit(self):
        ...
        # ADDED: atomic increment + decision under lock
        with self._count_lock:
            self.request_count += 1
            should_rotate = (
                self.tor_ctrl and
                self.request_count % self.rotate_every == 0
            )
        if should_rotate:
    ...
    # ADDED: save raw HTML per page for stylometry / audit
    def _save_raw_html(self, url, html, session_id):
        safe = re.sub(r'[^a-zA-Z0-9_-]', '_', url)[:80]
        folder = os.path.join(OUTPUT_DIR, f'session_{session_id}', 'raw_pages')
        ...
        try:
            ...
            warn(f"raw html save failed: {e}")
        return path

    # ADDED: renamed from check_tor_descriptor — original logic preserved
    def analyze_page_for_leaks(self, html, headers, url):
        """
        Analyzes page HTML for clearnet leaks: URLs, IPs, emails, server date.
        This is content analysis, NOT Tor descriptor parsing.
        """
        out = {
            'onion_address': url,
            'reachable': True,
        ...
        if headers:
            if headers.get('Server'):
                out['metadata']['server'] = headers['Server']
            if headers.get('Date'):
                out['metadata']['server_date'] = headers['Date']
                out['inconsistencies'].append(f"Server date exposed: {headers['Date']}")
        ...
        if clearnet:
            out['inconsistencies'].append(f'Clearnet URLs: {list(set(clearnet))[:5]}')
            out['metadata']['clearnet_refs'] = list(set(clearnet))

        ips = re.findall(
            r'\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b',
            html or ''
        )
        public = [ip for ip in ips if not ip.startswith(('127.', '192.168.', '10.', '172.'))]
        if public:
            out['inconsistencies'].append(f'Public IPs: {list(set(public))}')
            out['metadata']['exposed_ips'] = list(set(public))
        ...
        if emails:
            out['inconsistencies'].append(f'Clearnet emails: {list(set(emails))}')
            out['metadata']['clearnet_emails'] = list(set(emails))
...
def crawl_single(self, url, target_username=None, use_js=False):
    ...
    # ADDED: preserve HTML so callers don't need to re-fetch
    result['html'] = html
    # ADDED: save raw HTML for stylometry / audit
    result['raw_html_path'] = self._save_raw_html(url, html, self.session_id)
    ...
    # ADDED: banner grab (SOCKS-based)
    try:
        host = url.replace('http://', '').replace('https://', '').split('/')[0]
        intel['service_banners'] = {}
        for port in (80, 443):
            b = grab_banner(host, port)
            if b.get('banner'):
                intel['service_banners'][port] = b
    except Exception:
        intel['service_banners'] = {}

    # ADDED: server-status probing
    try:
        intel['server_status_hits'] = self.status_checker.check(url)
    except Exception:
        intel['server_status_hits'] = []

    # ADDED: TLS cert fetch (SOCKS-based)
    try:
        if url.startswith('https://'):
            host = url.replace('https://', '').split('/')[0]
            intel['ssl_info'] = fetch_tls(host)
    except Exception:
        intel['ssl_info'] = {'ssl_available': False}

    # ADDED: descriptor leak analysis
    try:
        intel['descriptor'] = self.analyze_page_for_leaks(
            html,
            dict(response.headers) if not isinstance(response, dict) else {},
            url
        )
    except Exception:
        intel['descriptor'] = {'inconsistencies': []}

    # ADDED: persist banner + descriptor rows
    for _port, _b in intel.get('service_banners', {}).items():
        try:
            self.db.save_banner(self.session_id, _b)
        except Exception:
            pass
    try:
        self.db.save_descriptor(self.session_id, intel.get('descriptor', {}))
    except Exception:
        pass
...
# DEPRECATED: use banner.grab_banner instead (broken on .onion hosts).
# Kept for backward compatibility only.
def grab_service_banner(self, host, port, timeout=5):
    """DEPRECATED: fails on .onion hosts. Use banner.grab_banner instead."""
    banner_data = {
    ...
    try:
        ...
        default_banners = ['Apache/2.4.41', 'nginx/1.18.0', 'OpenSSH_7.9',
                          'ProFTPD', 'Postfix ESMTP']
...
# DEPRECATED: use analyze_page_for_leaks instead
def check_tor_descriptor(self, onion_address):
    ...
    try:
        ...
        clearnet_refs = re.findall(
            r'https?://(?!.*\.onion)[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,}', html
        )
        if clearnet_refs:
            descriptor['inconsistencies'].append(
        ...
        emails = re.findall(
            r'[a-zA-Z0-9._%+-]+@(?!.*\.onion)[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}', html
        )
        if emails:
            descriptor['inconsistencies'].append(
...
# DEPRECATED: broken SOCKS+TLS combination. Use tls.fetch_tls instead.
def match_ssl_to_clearnet(self, onion_url):
    """DEPRECATED: broken SOCKS+TLS. Use tls.fetch_tls instead."""
    result = {
        'onion_url': onion_url,
        'clearnet_domains': [],
        'origin_server_hints': [],
...
def crawl_with_timeline(self, url, target_username=None):
    ...
    # FIXED: was re-fetching URL (already in self.visited → returned None).
    # Now reuses HTML preserved by crawl_single.
